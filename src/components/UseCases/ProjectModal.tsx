...
export const ProjectModal: React.FC<ProjectModalProps> = ({ project, onClose }) => {
  ...
  return createPortal(
    ...
    >
      <div 
        className="relative w-full max-w-2xl max-h-[88vh] bg-white dark:bg-[#121317] rounded-3xl border border-neutral-200 dark:border-neutral-800 shadow-2xl overflow-hidden flex flex-col"
        onClick={(e) => e.stopPropagation()}
      >
        {/* Scrollable Container with Hidden Scrollbar */}
        <div className="overflow-y-auto flex-1 scrollbar-none [scrollbar-width:none] [-ms-overflow-style:none] [&::-webkit-scrollbar]:hidden">
          {/* Close Button */}
          <button
            onClick={() => {
              sounds.playClick();
            ...
            className="absolute top-4 right-4 z-20 p-2.5 rounded-full bg-neutral-100/90 dark:bg-neutral-800/90 hover:bg-neutral-200 dark:hover:bg-neutral-700 text-neutral-600 dark:text-neutral-300 hover:text-neutral-900 dark:hover:text-white transition-colors backdrop-blur-sm shadow-md"
          ...
          <div className="p-6 sm:p-8 space-y-6">
            ...
            {project.metrics && (
              <div className="grid grid-cols-3 gap-3 p-4 rounded-2xl bg-neutral-50 dark:bg-neutral-900/60 border border-neutral-200 dark:border-neutral-800">
                {project.metrics.map((m, idx) => (
                  <div key={idx} className="text-center">
                    <div className="text-lg sm:text-xl font-bold text-google-blue">{m.value}</div>
                    <div className="text-[11px] text-neutral-500 uppercase tracking-wider">{m.label}</div>
                  </div>
                ))}
              </div>
            )}
            ...
            <div>
              ...
              <div className="flex flex-wrap gap-2">
                {project.techStack.map((tech, idx) => (
                  <span
                    key={idx}
                    className="px-3 py-1 rounded-full bg-neutral-100 dark:bg-neutral-800 text-neutral-800 dark:text-neutral-200 text-xs font-medium border border-neutral-200 dark:border-neutral-700"
                  >
                    {tech}
                  </span>
                ))}
            ...
            <div className="flex flex-wrap gap-3 pt-4 border-t border-neutral-200 dark:border-neutral-800">
              {project.demoUrl && (
                <a
                  ...
                  className="inline-flex items-center gap-2 px-6 py-3 rounded-full text-sm font-semibold text-white bg-google-blue hover:bg-google-hoverBlue transition-colors shadow-lg shadow-google-blue/20"
                >
